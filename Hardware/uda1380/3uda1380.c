 ...
 */
static const uint16_t uda1380_reg[UDA1380_CACHEREGNUM] = {
...
static inline unsigned int uda1380_read_reg_cache(struct snd_soc_codec *codec,
	unsigned int reg)
{
//	uint16_t *cache = codec->reg_cache;
...
static inline void uda1380_write_reg_cache(struct snd_soc_codec *codec,
	uint16_t reg, unsigned int value)
{
//	uint16_t *cache = codec->reg_cache;
	...
	unsigned int value)
{
	...
	if (!codec->active && (reg >= UDA1380_MVOL))
		return 0;
	printf("uda1380: hw write %x val %x\n", reg, value);
				 ...
				 struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_codec *codec = rtd->codec;
	uint16_t clk = uda1380_read_reg_cache(codec, UDA1380_CLK);

	/* set WSPLL power and divider if running from this clock */
	if (clk & R00_DAC_CLK) {
		int rate = params_rate(params);
		uint16_t pm = uda1380_read_reg_cache(codec, UDA1380_PM);
...
static int uda1380_modinit(void)
{
	int ret;
#if defined(CONFIG_I2C) || defined(CONFIG_I2C_MODULE)
	ret = i2c_add_driver(&uda1380_i2c_driver);
	if (ret != 0)
		printf("Failed to register UDA1380 I2C driver: %d\n", ret);
#endif
	return 0;
}

static void uda1380_exit(void)
{
#if defined(CONFIG_I2C) || defined(CONFIG_I2C_MODULE)
